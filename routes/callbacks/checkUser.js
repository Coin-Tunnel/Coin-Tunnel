...
sleep(1000).then(thing => {
    router.get("/", apiLimiter, async (req, res) => {
        ...
        if (req.session.checkUser > 30) return res.render("error", {error: "Request timed out! Try to sign up again, but this time be a little bit faster."})
          ...
          if (req.session.buser === "none") return res.render("error", {error: "You already have an account! Try signing in instead!"})
        ...
        const db = mongoclient.db("cointunnel");
              ...
              if (testresult === 1){
                ...
                return res.render("error", {error: "You already have an account! Try signing in instead."})
              }
      })
})
