...
sleep(1000).then(thing => {
  ...
  router.post("/trx", guiLimiter, async (req, res) => {
    ...
    if (Number(req.body.amount).toString().toLowerCase() === "nan") return res.send("That wasn't a valid amount!")
        ...
        var publicadr
        let userinfo = await checkStuff(mongoclient, req.session.buser);
        if (!userinfo.trx) return res.send("No TRX wallet connected!")
        if (userinfo.trx.address === "none"){
          return res.send("No wallet connected!");
        }
        ...
        if (balance<req.body.amount+1 || Number(req.body.amount) <  1) return res.send("You don't have enough funds to do this! Remember that the TRX network has network fees!")
        all()
        async function all(){
                // good
                let expiry = Date.now()+600000;
  ...
  router.post("/usdt", guiLimiter, async (req, res) => {
    if (!req.session.buser) return res.send("You must be logged in!");
    if (Number(req.body.amount).toString().toLowerCase() === "nan") return res.send("That wasn't a valid amount!")
        console.log(req.body)
        var secret;
        var publicadr
        let userinfo = await checkStuff(mongoclient, req.session.buser);
        if (!userinfo.trx) return res.send("No USDT wallet connected!")
        if (userinfo.trx.address === "none"){
          return res.send("No wallet connected!");
        }
          secret = userinfo.trx.privatex;
          ...
          let secretkeys = await decrypt(secret);
        if (req.body.address.length !== 34) return res.send("That wasn't a valid TRON USDT deposit address!")

        let wallet = await fetch("https://apilist.tronscan.org/api/account?address="+publicadr);
        wallet = await wallet.json();
        var balance = 0;

        if (wallet.trc20token_balances.find(e => e.tokenId === "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t")){
          balance = wallet.trc20token_balances.find(e => e.tokenId === "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t").balance/1000000;
        }else{
          balance = 0;
        }

        if (balance<req.body.amount+0.99 || Number(req.body.amount) <  1) return res.send("You don't have enough funds to do this! Remember that the USDT network has network fees!")
        all()
        async function all(){
                // good
                let expiry = Date.now()+600000;
         ...
         template = template.replaceAll("UserNameTemplate", userinfo.email);
         template = template.replaceAll("BuyerIdTemplate", userinfo.tunnelId);
         template = template.replaceAll("PutContentHereTemplate", `<h1 style="text-align: center;">Hello ${userinfo.email}!</h1><p style="text-align: center;">There was a recent attempt to withdraw ${req.body.amount} USDT from your account!&nbsp;<br />If this was you, great! Click CONFIRM below!</p>`)
         template = template.replaceAll("AmountTemplate", req.body.amount);
         template = template.replaceAll("AddressTemplate", req.body.address);
         template = template.replaceAll("ConfirmationCodeTemplate", "https://www.coin-tunnel.ml/validate/"+randomid)
         const mailOptions = {
