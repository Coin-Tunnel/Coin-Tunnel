...
sleep(1000).then(thing => {
    ...
    router.get("/", guiLimiter, async (req, res) => {
      ...
      txlist = txlist.reverse();
        res.render("merchants/wallets.ejs", {db: user, key: keys, wallet: wallets, transac: transactions, transactionList: txlist});
    })
    router.get("/transactions", guiLimiter, async (req, res) => {
        var keys = {};
          var wallets = "No wallet connected";
          if (!req.session.muser) return res.redirect("/signin-m");
          let user = await mongoclient.db("cointunnel").collection("merchantData")
        ...
        txlist = txlist.reverse();
          res.render("merchants/transactions.ejs", {db: user, key: keys, wallet: wallets, transac: transactions, transactionList: txlist});
    })
    router.get("/keys", guiLimiter, async (req, res) => {
        var keys = {};
          var wallets = "No wallet connected";
          if (!req.session.muser) return res.redirect("/signin-m");
          let user = await mongoclient.db("cointunnel").collection("merchantData")
        ...
        txlist = txlist.reverse();
          res.render("merchants/apikeys.ejs", {db: user, key: keys, wallet: wallets, transac: transactions, transactionList: txlist});
    })
    router.get("/settings", guiLimiter, async (req, res) => {
        var keys = {};
          var wallets = "No wallet connected";
          if (!req.session.muser) return res.redirect("/signin-m");
          let user = await mongoclient.db("cointunnel").collection("merchantData")
        ...
        txlist = txlist.reverse();
          res.render("merchants/settings.ejs", {db: user, key: keys, wallet: wallets, transac: transactions, transactionList: txlist});
    })
})
